...
|---|---|---|---|---|
| 1 | No session has ever armed a watch | Start a fresh Claude Code session on this install; before running the Monitor command the SessionStart hook prints, run: `scripts/healthcheck.py \| grep -A2 '^watch'` | `watch        no session has armed a watch; unread bytes with no session open is normal` | Those two lines. |
| 2 | A session ran the hook and never armed | Start a second session (so its SessionStart hook writes a `pending` registry entry) and deliberately do not run the Monitor command it prints; from either session run the same `healthcheck.py` grep | A `watch` line matching state 1, 3, or 4 (whichever else is true), plus a line directly under it: `             N session(s) ran the hook and never armed` | The full multi-line `watch` block, including the count. |
