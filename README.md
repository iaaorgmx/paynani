...
Paynani es para una persona que quiere que su agente tenga correo propio, pero no quiere convertir su buzón en una puerta abierta. El lector principal de este README eres tú: quien decide si instala, qué cuenta entrega, quién queda en `roster.md` y qué pruebas necesita ver antes de confiar.

La documentación para el agente vive en [`AGENTS.md`](AGENTS.md). El README se queda con la decisión humana: qué hace Paynani, qué cambia en tu computadora, qué riesgos existen y dónde leer el detalle profundo.
...
- detectar eventos de correo con IMAP IDLE;
- escribir eventos canónicos a `state/events.jsonl`;
- entregar eventos por cursor a un runtime soportado;
- aplicar coincidencias exactas de `roster.md` y notificadores declarados;
- enviar correo mediante `scripts/send.sh` dentro de la frontera del roster.

Paynani **no** es responsable de:

- configurar por sí solo rutas estáticas del gateway de Hermes;
- probar la identidad real de un remitente más allá del correo recibido y las validaciones del proveedor;
- decidir qué hace un agente después de recibir una instrucción autorizada;
- administrar políticas del proveedor de correo;
- reemplazar supervisión real del sistema operativo.
...
La doctrina es la misma que en las pruebas humanas: que el agente diga “no hay correo nuevo” no basta. Primero tiene que poder demostrar que está escuchando.
...
Por eso el último UID visto se perPor eso el último UID visto se guarda por mensaje, `UIDVALIDITY` se revisa en
