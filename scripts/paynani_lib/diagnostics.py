...
def doctor_data() -> dict:
    ...
    if runtime.get("selected") is None:
        checks.append(_check("runtime", "blocked", "no runtime is selected", runtime, "PAYNANI_RUNTIME=openclaw scripts/healthcheck.py"))
    ...
    if q.get("damaged_at") is not None:
        checks.append(_check("queue", "blocked", f"event journal is damaged at byte {q['damaged_at']}", q, "cp state/events.jsonl state/events.jsonl.bak"))
