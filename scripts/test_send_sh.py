...
def write_common(tmp: pathlib.Path, himalaya_version: str):
    bin_dir = tmp / "bin"
    bin_dir.mkdir()
...
def run_send(himalaya_version: str, *args: str):
    with tempfile.TemporaryDirectory() as raw:
        tmp = pathlib.Path(raw)
        env, body, log = write_common(tmp, himalaya_version)
...
run, calls = run_send("himalaya v1.2.0")
check("send.sh refuses Himalaya v1 before SMTP", run.returncode != 0 and "paynani needs v2.x to send" in run.stderr and "v1.x has no `smtp send`" in run.stderr, run.stderr)
check("Himalaya v1 refusal does not attempt smtp send", "smtp send" not in calls and calls.strip() == "--version", calls)
